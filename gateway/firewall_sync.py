...
def render(pairs):
    rules = [
        'table inet hotelcast {',
        ' chain forward {',
        '  type filter hook forward priority 0; policy accept;',
        '  ct state established,related accept',
    ]

    # As regras de autorização precisam vir ANTES do drop geral.
    for p in pairs:
        g = p['guest_ip']
        c = p['cast_ip']
        rules.append(f'  ip saddr {g} ip daddr {c} tcp dport {{8008,8009,8443}} accept')
        rules.append(f'  ip saddr {g} ip daddr {c} udp accept')
        rules.append(f'  ip saddr {c} ip daddr {g} accept')

    rules.append('  ip saddr ' + GUEST_NET + ' ip daddr ' + CAST_NET + ' drop')
    rules += [' }', '}']
    return '\n'.join(rules) + '\n'
